...
#regressor = linear_model.Ridge( alpha = 0.00009)
...
for bins in range( 1, 6 ):
    ...
    # print ( tot )
   # X = new_train.drop('SalePrice',1)

    
    #print X


    kmeans = KMeans(n_clusters=bins, max_iter=100).fit(X)
    pred = kmeans.predict(X)
    #print pred
    #exit()
    new_house_data =pd.DataFrame()
    new_house_data = new_train
    new_house_data['bin']= pred
    #print new_house_data.head()
    
    bin_data = new_house_data.groupby('bin')
    #print bin_data
    bin_labels = new_house_data['bin']
...
#     exit()
    ...
    partition_bin = helpers.create_partitions_for_k_fold_validation( bins, folds, bin_data, predict_column )
...
#             exit()
    ...
    plt.title( 'Actual Vs. Predicted Sale Price - Correlation {}'.format( predictions_df['SalePrice'].corr( predictions_df['fold_predictions'] ), r2_score( predictions_df['SalePrice'], predictions_df['fold_predictions'] ) ) )
    plt.show()
